...
// wether mouse is withn the bounds
...
// creaates new blank image
func NewBlankImage(cols, rows int, c color.Color) image.Image {
	img := image.NewNRGBA(image.Rect(0, 0, cols, rows))

	// go through each pixel of image and set color
	for y := 0; y < rows; y++ {
		for x := 0; x < cols; x++ {
			img.Set(x, y, c)
		}
...
func (pxCanvas *PxCanvas) SetColor(c color.Color, x, y int) {
	// we check the type for pixelData to avoid errs
...
// to load a blank or existing image
